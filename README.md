...
## はじめに
　AorUIControllerはNPCダイアログを改造したもので、NPCと対話した際のNPCの名前やセリフからコードを取得してそのUIをカスタムできるようにしたシステムです。  
　なお、JSON JIは将来 [OreUI](https://github.com/Mojang/ore-ui) に置き換えられる可能性があるため、使用できなくなる可能性があります。また、一部のコードは既存のコード、例えばボタンやダイアログなどはテンプレートに依存している為、ゲームのアップデートで変更が生じた場合は使用できなくなる可能性にも留意して下さい。
